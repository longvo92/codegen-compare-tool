...
```bash
pip install "codegen-compare-tool[viewer] @ git+https://github.com/longvo92/codegen-compare-tool.git"   # hoặc: pip install PySide6
```
...
Các rule sẵn có hiểu output của Embedded Coder. Nhóm gen từ TargetLink hay DaVinci sinh ra churn khác — một element `<TL-CHECKSUM>`, một banner tool kèm số build — mà rule sẵn có không nhận ra, nên mỗi lần regenerate nó hiện thành thay đổi thật. `--rules FILE.json` dạy tool các pattern đó mà không phải sửa source.

Các rule là **cộng thêm** (additive): chạy trên nền rule sẵn có, không thay thế. Không truyền `--rules` thì hành vi y như trước, và project có dùng vẫn nhận đủ mọi filter sẵn có.

File rule là một list JSON, hoặc một object có key `rules`:
...
- `name` — nhãn mà một hunk do rule này giải thích sẽ hiện dưới nhãn đó, trong diff và trong số đếm. Nó đọc là **Unimportant**, không bao giờ là Comment (Comment là hạng mục sẵn có với hành vi report riêng).
- `pattern` — một regular expression Python. Mọi match bị bỏ khỏi *shadow* — chỗ quyết định verdict — nên một khác biệt rule này giải thích trọn vẹn thì hết là thay đổi thật.
- `replacement` — mỗi match biến thành gì (mặc định: xoá hẳn). Phải nằm trên một dòng.
- `extensions` — rule áp cho loại file nào (mặc định: tất cả). Dấu chấm đầu không bắt buộc.

Vẫn đúng luật fail-safe như mọi chỗ khác: **một filter không bao giờ giấu được thay đổi thật.** Cái gì rule không giải thích trọn vẹn thì vẫn là thay đổi thật, rule có regex không compile được thì bị bỏ qua kèm cảnh báo chứ không làm hỏng cả lần chạy, và rule làm đổi số dòng của file thì bị bỏ qua (diff cần shadow khớp từng dòng với file). Rule áp cho **cả** report lẫn viewer, nên hai bên vẫn cho cùng đáp án.
...
Cơ chế này áp dụng cho cả ARXML và A2L, không riêng C: một
`APPLICATION-SW-COMPONENT-TYPE` hay một `CHARACTERISTIC` bị regenerate emit ra
theo thứ tự khác là một block `moved`, và vì bước so khớp chạy trên shadow, một
`UUID` mới trên element bị dịch chỗ cũng không làm nó hết ghép được.
